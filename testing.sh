...
# SCPT_1.10: Now the Installer Script is independent of the existence of DLNA Media Server, DLNA MediaServer is a optional package. Now You can see the installation logs in: /tmp/wrapper_ffmpeg.log. 
...
if [[ $EUID -ne 0 ]]; then
  ...
  error "YOU MUST BE ROOT FOR EXECUTE THIS INSTALLER. Please write ("${PURPLE}" sudo -i "${RED}") and try again with the Installer." >> $logfile
  exit 1
fi
